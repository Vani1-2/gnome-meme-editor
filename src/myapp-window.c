...
on_template_selected (GtkFlowBox *flowbox, GtkFlowBoxChild *child, MyappWindow *self)
{
	...
	if (!template_path) {
		g_warning ("No template path found");
		return;
	}

	g_print ("Loading template: %s\n", template_path);
...
on_drag_begin (GtkGestureDrag *gesture, double x, double y, MyappWindow *self)
{
	...
	if (self->user_image != NULL) {
		double layer_left = self->image_layer.x - (self->image_layer.width * self->image_layer.scale) / 2.0 / img_width;
		double layer_right = self->image_layer.x + (self->image_layer.width * self->image_layer.scale) / 2.0 / img_width;
		double layer_top = self->image_layer.y - (self->image_layer.height * self->image_layer.scale) / 2.0 / img_height;
		double layer_bottom = self->image_layer.y + (self->image_layer.height * self->image_layer.scale) / 2.0 / img_height;
